...
snapshot `results/integrated_v21.json`.
Previous locked entry: `sprint5_v9` (2.7182, `results/v9_locked.json`) —
still the per-case fallback inside solve().

| Metric | Value |
|--------|-------|
| Validation score (RF=1.0) | **1.6507** (v9: 2.7182) |
| Feasible cases | **100 / 100** |
| Average runtime | **~0.238s/case** (max ~0.87s) |
| Runtime-adjusted total @ median 1s | **1.311** (v20: 1.342 same-window; v9: 2.11) |
...
39% better raw quality, and the kept v21 ordering anchor improves v20 at medians
{1,2,3}s.
...
results/                     # Curated result artifacts (integrated_v21.json = current)
...
.venv/bin/python scripts/check_public_release.py    # PASS (defaults: results/integrated_v21.json, --max-score 1.655)
```
