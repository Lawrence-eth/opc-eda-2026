...
snapshot `results/integrated_v23.json`.
Previous locked entry: `sprint5_v9` (2.7182, `results/v9_locked.json`) —
still the per-case fallback inside solve().

| Metric | Value |
|--------|-------|
| Validation score (RF=1.0) | **1.6385** (v9: 2.7182) |
| Feasible cases | **100 / 100** |
| Average runtime | **~0.246s/case** (max ~0.90s) |
| Runtime-adjusted total @ median 1s | **1.314** (v22: 1.338; v9: 2.11) |
...
40% better raw quality, and the kept v23 width pockets improve v22 at medians
{1,2,3}s.
...
results/                     # Curated result artifacts (integrated_v23.json = current)
...
.venv/bin/python scripts/check_public_release.py    # PASS (defaults: results/integrated_v23.json, --max-score 1.64)
```
