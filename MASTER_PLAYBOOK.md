...
- **Quality work is CLOSED — no more variants (that's now busywork, I.0).**
- **N9 robustness — DONE:** v9 ran on 5,000 FloorSet *training* instances → 100% feasible, runtime bounded (artifact: `results/n9_robustness.json`). The catastrophic hidden-set-infeasibility risk is mitigated. (Note: a few coarse high-count structural gates exist, e.g. n∈{117–120}; feasibility-validated by this run.)
- **Timeline:** presentation ≈2026-06-13 (v9 ready now), real deadline ≈2026-07-13. Remaining is presentation prep, not more quality attempts.
