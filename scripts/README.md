...
| [`train_structured_predictor.py`](train_structured_predictor.py) | Structured model training | Build/reuse hash-bound supervision shards and train the CPU-sized shape, rank, horizontal relation, and vertical support baseline. Contest runs require the union of clean-v1, raw-v1, and sealed-v2 source exclusions. |
| [`evaluate_structured_predictor.py`](evaluate_structured_predictor.py) | Structured holdout evaluation | Report topology accuracy, exactness, feasibility, RF=1 cost, runtime, confidence fallback, and offline v32-best-of on development v1 folds. It refuses sealed manifests before parsing them. |
| [`evaluate_structured_public.py`](evaluate_structured_public.py) | Structured public evaluation | Measure produced-placement feasibility/cost/runtime on the 100 public cases and keep offline best-of clearly non-deployable. |
