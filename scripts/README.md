...
| [`build_dual_parent_cache.py`](build_dual_parent_cache.py) | Build or independently validate schema-v2 dual-parent research shards. | Source-file train/development/calibration partitions exclude the complete configured holdout union. Numeric NPZ shards contain no paths, offsets, raw group IDs, strings, or Python objects; those identities remain in a hash-bound manifest. Every selected layout must survive exact oracle decode before a Linux atomic no-replace publication. |
