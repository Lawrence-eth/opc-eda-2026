...
| [`build_dual_parent_cache.py`](build_dual_parent_cache.py) | Build or independently validate schema-v3 dual-parent research shards. | Production always imports `FloorplanDatasetLite` from the verified pinned data-root bytes; no caller dataset is accepted. Full validation requires an expected manifest digest and replays every source offset. Numeric NPZ shards remain identity-free, canonical, and `allow_pickle=False`; publication is Linux atomic no-replace. |
